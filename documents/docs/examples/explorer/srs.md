...
이 문서는 RIGO Blockhcain 의 Block Explorer 인 **ArcaScan** 에 대한 구성 요구사항 및 기능 요구사항에 대하여 기술한다.
...
ArcaScan 은 RIGO 네트워크에서 생성되는 블록, 트랜잭션 그리고 계정 관련 상태 변경 정보를
...
RIGO 네트워크와의 연결을 동한 정보 획득, 획득된 정보의 DB 저장을 통한 동기화, 그리고 DB 정보에 기반한 사용자 요청 응답으로 기본 기능을 구성한다.

## Initialization from Genesis
RIGO 블록체인 원장의 최초 상태 정보(Genesis)가 ArcaScan DB에 반영되어야 한다.  
RIGO Genesis 에는 블록체인 시작 시점에서의 검증노드(Validator), 계정(Account)별 자산(Balance) 정보등이 포함되어 있다.
따라서 ArcaScan 최초 실행시, 스캔 대상이 되는 RIGO 블록체인 네트워크의 Genesis 정보로 ArcaScan DB의 초기 상태를 구성해야 한다.
...
ArcaScan DB는 RIGO 블록체인 원장과 동기화 되어야 한다.  

RIGO 블록체인 원장과의 동기화는 RIGO 블록체인 원장에 기록되는 모든 상태 변경 정보를 수신하여 이를 ArcaScan DB에 재구성함을 의미 하는 것으로,
...
RIGO RPC 호출 대한 자세한 정보는 [RWeb3 API](../../api/rweb3.md) 를 참조한다.
...
ArcaScan 실행 중 RIGO 네트워크에서 신규로 생성되는 모든 블록과 트랜잭션은 **이벤트 구독** 방식을 통해 실시간에 가깝게 ArcaScan DB 와 동기화 되어야 한다.
...
RIGO 블록체인은 이미 블록번호 `bn1` 까지 생성된 상황이 가능하다.  
때문에 ArcaScan 는 시작시, 
DB에 반영된 마지막 블록 번호(`bn0`)와 
현재 RIGO 블록체인의 마지막 블록 번호(`bn1`) 를 조회 하여,
...
예를 들어, DB에 반영된 마지막 블록 번호가 `m` 이고, RIGO 블록체인의 마지막 블록 번호가 `n, (n>m)` 으로 조회 되었을 경우, 
블록 `n`까지 블록을 조회 하여 이를 DB에 반영하는 사이에 RIGO 네트워크에서 블록 `n+1` 이 생성될 가능성이 있다.  
