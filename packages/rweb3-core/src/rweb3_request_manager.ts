...
import {
    ...
    RWeb3APIRequest,
    RWeb3APIReturnType,
    RWeb3APIPayload, JsonRpcPayload, JsonRpcResponse, JsonRpcResponseWithError, JsonRpcError, JsonRpcBatchResponse
} from "rweb3-types";
import {ContractExecutionError, InvalidResponseError, ResponseError, RpcError, rpcErrorsMap} from 'rweb3-errors';
...
export class RWeb3RequestManager<API extends RWeb3APISpec = RigoExecutionAPI> {
    ...
    >(request: RWeb3APIRequest<API, Method>): Promise<ResponseType> {
        ...
        let response = await provider.request<Method, ResponseType>(
